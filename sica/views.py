...
def transaccion_IVA(request, id_partida):
    ...
    formulario.fields['transaccion_iva'].queryset = Transaccion.objects.filter(id_partida=id_partida).exclude(Q(id_subCuenta="1105.01") | Q(id_subCuenta="2105.01"))
...
def estadoCapital(request):
    ...
    subCuentasResultado = SubCuenta.objects.filter((Q(debe__gt=0.00) | Q(haber__gt=0.00)) &
                                                   ((Q(id_subCuenta__contains='4101') | Q(id_subCuenta__contains='4102') | Q(id_subCuenta__contains='4201') | Q(id_subCuenta__contains='4202')) |
                                                   (Q(id_subCuenta__contains='5101') | Q(id_subCuenta__contains='5201') | Q(id_subCuenta__contains='5202'))))
    ...
    if suma_debe < suma_haber:
        subCuentaUtilidad.haber = utilidad * 1 #antes 0.6 por reserva
    else:
        subCuentaUtilidad.debe = utilidad * 1 #antes 0.6 por reserva
...
def balanceGeneral(request):
    ...
    if suma_debe < suma_haber:
        subCuentaUtilidad.haber = utilidad * 0 #antes 0.6 excedente de reserva
    else:
        subCuentaUtilidad.debe = utilidad * 0 #antes 0.6 excedente de reserva
